...
/* STEP 0 */ import {useState} from "react";
...
export default function Input() {
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    /* STEP 2 */ color: inputValue > 10 ? 'crimson' : 'royalblue'
  };

  return (
    <div className='widget-input container'>
      <h2>Input</h2>
      {/* STEP 3 */} <div id='output' style={style}>{inputValue.toUpperCase()}</div> 
      <div>
        {/* STEP 6 */} <input id='input' type='text' value={inputValue} onChange={changeInput} /> 
